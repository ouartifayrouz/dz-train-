...
class _ProfilePageState extends State<ProfilePage> {
            ...
            _buildListTile(Icons.notifications, local.notifications, () {
              if (userData != null) {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => NotificationsScreen(username: userData!['username']!),
                  ),
                ).then((_) => fetchUnreadNotificationsCount());
              }
            }, isDark, badgeCount: _unreadNotifications),
            _buildListTile(Icons.history, local.favoriteTrips, () {
              Navigator.push(context, MaterialPageRoute(builder: (_) =>  HistoriqueTrajetsPage(username: widget.username)));
            }, isDark),
            _buildListTile(Icons.train, local.trainSchedules, () {
              Navigator.push(context, MaterialPageRoute(builder: (_) => HoraireTrainPage()));
            }, isDark),
            ...
            _buildListTile(Icons.logout, local.logout, () async {
              final confirmed = await showDialog<bool>(
                ...
                builder: (context) => AlertDialog(
                  title: Text(local.logout, style: TextStyle(color: isDark ? Colors.white : Colors.black)),
                  content: Text(local.logoutConfirmation),
                  actions: [
                    TextButton(onPressed: () => Navigator.pop(context, false), child: Text(local.cancel)),
                    TextButton(onPressed: () => Navigator.pop(context, true), child: Text(local.logout)),
                  ],
                ),
              );
              if (confirmed == true) {
  ...
  Widget _buildListTile(IconData icon, String title, VoidCallback onTap, bool isDark, {int badgeCount = 0}) {
    final color = isDark ? Colors.white : Colors.black;

    Widget leadingIcon = badgeCount > 0
        ? external_badges.Badge(
